...
public class scrML_NA_FrequencyEncoding : _IEncodingGenerator<float>
{
    private Dictionary<string, float> _tagIntegerMapping;
    private bool normalizeValues = true;
    ...
    protected override bool GenerateGene(in string filePath, out Gene<float> gene)
    {
        ...
        Dictionary<string, float> frequencyCount = new Dictionary<string, float>();
        XmlDocument doc = new XmlDocument();
        doc.Load(filePath);
        XmlNodeList nodes = doc.SelectNodes("//*");
        int total = 0;
        foreach (XmlNode node in nodes)
        {
            ...
            {
                string tag = node.Name;
                if (!frequencyCount.TryAdd(tag, 1))
                {
                    frequencyCount[tag]++;
                }
            }
        ...
        if (normalizeValues)
        {
            foreach (var key in frequencyCount.Keys.ToList())
            {
                frequencyCount[key] /= total;
            }
        }
        ...
        foreach(XmlNode node in nodes)
        {
            ...
            {
                string tag = node.Name;
                geneList.Add(frequencyCount[tag]);
            }
        }
